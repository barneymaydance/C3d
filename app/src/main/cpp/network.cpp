...
//************************************************************************
ThreeDConvLayer::ThreeDConvLayer(string name, int D,int H,int W,int inCh,int outCh,int *strides, int *padding) : Layer(name) {
    _D=D;
    _H=H;
    _W=W;
    _inCh=inCh;
...
void ThreeDConvLayer::forward() {
    ...
    __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,"value of certain point is %s",   to_string(temp(11,11)).c_str());
    //add bias

    temp.rowwise()+=_bias;

    temp.array() = temp.array().cwiseMax(0);
...
void ThreeDConvLayer::im2col(){
    ...
    for (int Din = 0; Din < (_inputShape[0] + _padding[0] * 2 - _D) + 1; Din= Din+_strides[0]) {
        for (int Hin = 0; Hin < (_inputShape[1] + _padding[1] * 2 - _H)  + 1; Hin= Hin+_strides[1]) {
            for (int Win = 0; Win < (_inputShape[2] + _padding[2] * 2 - _W) + 1; Win= Win+_strides[2]) {
...
string network::predict(Eigen::Matrix<float, Dynamic, Dynamic> image) {
    Eigen::Matrix <float, Dynamic,Dynamic> finaloutput;
    for (int i = 0; i < layers.size(); i++) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,"Layer %s proccessing",  layers[i]->_name.c_str());
        if (i == 0) {
        ...
        if(i==layers.size()-1){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,"last output %s",  to_string(finaloutput(0,511)).c_str());
        }
    }

    return to_string(finaloutput(0,511));
}
...
void MaxThreeDPooling::forward() {
    ...
    for (int CHin = 0; CHin < col; ++CHin) {
        int row =0;
        for (int Din = 0; Din < (_inputShape[0] + _padding[1] - _ksize[1]) + 1; Din= Din+_strides[1]) {
            for (int Hin = 0; Hin < (_inputShape[1] + _padding[2] - _ksize[2])  + 1; Hin= Hin+_strides[2]) {
                for (int Win = 0; Win < (_inputShape[2] + _padding[3] - _ksize[3]) + 1; Win= Win+_strides[3]) {
                    //inside for the filter kernel
                    float value= -FLT_MAX ;
    ...
    __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,"value of certain point is %s",   to_string(_output(11,11)).c_str());
}
